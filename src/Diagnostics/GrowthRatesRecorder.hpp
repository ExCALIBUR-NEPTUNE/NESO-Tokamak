#ifndef TOKAMAK_GROWTH_RATES_RECORDER_H
#define TOKAMAK_GROWTH_RATES_RECORDER_H
...
public:
    GrowthRatesRecorder(const LU::SessionReaderSharedPtr session,
                        int prob_ndims, std::shared_ptr<T> n,
                        std::shared_ptr<T> w, std::shared_ptr<T> phi, int npts,
    ...
    inline double compute_Gamma_a()
    {
        Array<OneD, NekDouble> integrand(m_npts);
        // Set integrand = n - ϕ
        Vmath::Vsub(m_npts, m_n->GetPhys(), 1, m_phi->GetPhys(), 1, integrand,
                    1);
