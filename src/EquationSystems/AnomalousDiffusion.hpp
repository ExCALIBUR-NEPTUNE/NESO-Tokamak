...
{
class AnomalousDiffusion : public TokamakSystem
{
public:
    friend class MemoryManager<AnomalousDiffusion>;
        ...
        const SD::MeshGraphSharedPtr &graph)
    {
        SU::EquationSystemSharedPtr p =
            MemoryManager<AnomalousDiffusion>::AllocateSharedPtr(session,
                                                                 graph);
        p->InitObject();
        return p;
    }
    ~AnomalousDiffusion() override = default;

protected:
    AnomalousDiffusion(const LU::SessionReaderSharedPtr &session,
                       const SD::MeshGraphSharedPtr &graph);
